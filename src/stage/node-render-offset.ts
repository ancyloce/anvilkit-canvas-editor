 ...
 * top-left transform.
 *
 * `Konva.Ellipse` is centered at `(x, y)`, so `CanvasNodeRenderer` draws it at
 * `transform + bounds/2`; every other node renders at its top-left, so their
 * Konva position equals the IR transform.
 *
