...
describe("polygonTool", () => {
	it("sets draft on pointerdown, updates on pointermove, commits node.create on pointerup", () => {
		const h = makeHarness();
		polygonTool.onPointerDown?.(pointerEvent(10, 20), h.ctx);
		...
		polygonTool.onPointerMove?.(pointerEvent(50, 80), h.ctx);
	...
	it("drawing right-to-left or bottom-to-top works (min/abs derive corner)", () => {
		const h = makeHarness();
		polygonTool.onPointerDown?.(pointerEvent(100, 100), h.ctx);
		polygonTool.onPointerUp?.(pointerEvent(30, 40), h.ctx);
		expect(h.commits).toHaveLength(1);
		const cmd = h.commits[0] as CanvasNodeCreateCommand;
	...
	it("skips commit when end point is within MIN_DIMENSION of start (degenerate click)", () => {
		const h = makeHarness();
		polygonTool.onPointerDown?.(pointerEvent(10, 10), h.ctx);
		polygonTool.onPointerUp?.(pointerEvent(10.5, 10.5), h.ctx);
	...
	it("pointermove without a prior pointerdown is a no-op", () => {
		const h = makeHarness();
		expect(() =>
			polygonTool.onPointerMove?.(pointerEvent(10, 10), h.ctx),
		).not.toThrow();
		expect(h.ctx.draftStore.getState().draft).toBeNull();
	});
	...
	it("onDeactivate clears draft + guides", () => {
		...
		polygonTool.onDeactivate?.(h.ctx);
		expect(h.ctx.draftStore.getState().draft).toBeNull();
		expect(h.ctx.guidesStore.getState().guides).toEqual([]);
	});
});
