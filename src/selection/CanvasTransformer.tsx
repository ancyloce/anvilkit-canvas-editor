...
/**
 * Scaled size of the rotation icon's 24×24 viewBox. The lucide `refresh-ccw` art
...
export function CanvasTransformer(): React.JSX.Element | null {
  ...
  // Live rotation readout while rotating: shows the angle (normalized to
  // (-180, 180]) and tracks the cursor. `setPointersPositions` runs on every
  // window mousemove of the rotate drag, so the stage pointer is current; we
  // place the badge in layer/design coords (counter-scaled to a constant
  // on-screen size) just down-right of the cursor.
