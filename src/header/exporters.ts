...
// The download path itself lives in `download-artifact.ts` — a leaf with no
// stage/layout/brand imports, so `CanvasStudio.tsx`'s recovery export can reuse
// it without pulling this module's rasterizer into the eager chunk. Re-exported
// here because this is where every existing importer looks for these names.
import { sanitizeExportFilename } from "./download-artifact.js";
import type {
